...
int main(int argc, char **argv)
{
  ...
  {
    /* make sure each option starts with '-' */
    if( argv[argind][0]!='-' ) {
    ...
    if( argv[argind][2] )	argpar= argv[argind]+2; // e.g. "0" in "-C0"
    else {
      if( argind+1 >= argc ) argpar= NULL;
      else argpar= argv[argind+1]; // but some toggle option should set "argpar" to NULL again
    }
